...
export const getObject = (obj = {}, path = [], filterUndefined = [undefined, ""]) => {
...
export const convertToUndefined = (obj = {}, filterUndefined = [undefined, ""]) => {
    let tempObj = {}
...
export const historyRoute = (to, from) => { // 记录了之后， 对于各种情况导致的重定向登录页的时候，等登录完成后可以回过去,当对于路由被拦截对此重定向之后回失效，所以不做此用； 或者说 可以做用户的页面活动（浏览记录之类的)
    ...
    if (localRouter.length) { // 如果有记录了 做下处理
        if (localRouter[localRouter.length - 1].to.name === to.name) { // 上次保存的跟这一次保存的是同一个路由（处理用户的刷新问题)
