  ...
  return new Promise((resolve, reject) => {
    getWXConfig( window.location.href.split('#')[0] // encodeURIComponent(window.location.href.split("#")[0])
      ...
      wx.config({
        ...
        jsApiList: jsApiList || ['onMenuShareTimeline', 'onMenuShareAppMessage', 'chooseWXPay'] // 必填，需要使用的JS接口列表
      })
      wx.ready(() => {
        resolve("OK")
      })
  ...
  methods: { // 混入几个跟分享相关的方法
    ...
    shareMixin(to, from, vm) {
      ...
      */
      ...
      let { path = '/home', meta: { shareInfo: { paramsKey = [], sharePath = path, ...others } = { } } = {} } = to
      let referrer = window.localStorage.getItem('userId') || ""
