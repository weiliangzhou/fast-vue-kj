...
export const salesmanLogin = () => {
  if (isWX) {
    ...
    }/salesman-login&response_type=code&scope=snsapi_userinfo&state=login#wechat_redirect`;
  } else {
    // 在微信体制外登录则不需要跳转到微信授权，直接通过手机号码登录
...
export const SilentToLogin = () => {
  ...
  let uri = getQueryString("referrer")
    ? redirectUri.split("?")[0] + "?referrer=" + getQueryString("referrer")
    : redirectUri.split("?")[0];
  ...
  function jsApiCall() {
    ...
    WeixinJSBridge.invoke("getBrandWCPayRequest", config, function(res) {
      if (res.err_msg == "get_brand_wcpay_request:ok") {
        successCallback &&
          successCallback({ result: 0, orderNo, message: "支付成功" });
